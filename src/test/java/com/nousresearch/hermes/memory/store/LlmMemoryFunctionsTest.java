...
class LlmMemoryFunctionsTest {
    ...
    @Test
    @DisplayName("factExtractor respects maxFacts limit")
    void extractFactsWithLlm_maxFactsLimit() {
        ModelClient mockClient = mock(ModelClient.class);
        when(mockClient.chatCompletion(anyList(), anyList(), anyBoolean(), anyMap()))
            .thenReturn(responseWithContent(
                "- Fact one\n- Fact two\n- Fact three\n- Fact four\n- Fact five"));
    ...
    @Test
    @DisplayName("factExtractor falls back to summary when LLM fails")
    void extractFactsWithLlm_exception_fallsBack() {
        ModelClient mockClient = mock(ModelClient.class);
        when(mockClient.chatCompletion(anyList(), anyList(), anyBoolean(), anyMap()))
        ...
        List<String> facts = funcs.factExtractor().extract("Some summary text", 3);
        assertEquals(1, facts.size());
        assertEquals("Some summary text", facts.get(0));
    }

    @Test
    @DisplayName("factExtractor returns empty for blank summary")
    void extractFactsWithLlm_blankInput() {
        ModelClient mockClient = mock(ModelClient.class);
        var funcs = new LlmMemoryFunctions(mockClient);

        List<String> facts = funcs.factExtractor().extract("", 5);
        assertTrue(facts.isEmpty());
        verifyNoInteractions(mockClient);
    }

    @Test
    @DisplayName("factExtractor returns empty when maxFacts is 0")
    void extractFactsWithLlm_maxFactsZero() {
        ModelClient mockClient = mock(ModelClient.class);
        var funcs = new LlmMemoryFunctions(mockClient);

        List<String> facts = funcs.factExtractor().extract("Some text", 0);
        assertTrue(facts.isEmpty());
        verifyNoInteractions(mockClient);
    }

    @Test
    @DisplayName("Constructor accepts model aliases")
    void constructor_withModelAliases() {
        ModelClient mockClient = mock(ModelClient.class);
        var funcs = new LlmMemoryFunctions(mockClient, "fast", "smart");
        assertNotNull(funcs);
        assertNotNull(funcs.summaryFunction());
        assertNotNull(funcs.factExtractor());
    }
}
