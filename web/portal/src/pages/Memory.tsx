...
const DECAY_STAGES = [
  { key: "full", labelKey: "memory.fullStage", color: "#34c759", bg: "rgba(52,199,89,0.12)" },
  { key: "warm", labelKey: "memory.warmStage", color: "#ff9500", bg: "rgba(255,149,0,0.12)" },
  { key: "cool", labelKey: "memory.coolStage", color: "#5ac8fa", bg: "rgba(90,200,250,0.12)" },
  { key: "evicted", labelKey: "memory.evictedStage", color: "#ff3b30", bg: "rgba(255,59,48,0.12)" },
];
          ...
          <div className="space-y-4">
            <GlassCard>
              <div className="mb-4 flex items-center justify-between">
                ...
                <button onClick={loadStats} className="text-primary active:scale-95 transition">
                ...
                {Object.keys(stats.skill.byScope ?? {}).length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-2">
                    {Object.entries(stats.skill.byScope).map(([scope, count]) => (
                      <span key={scope} className="rounded-full bg-muted/60 px-2 py-0.5 text-[11px] text-muted-foreground">
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl bg-muted px-4 py-2.5 text-[13px] text-foreground placeholder:text-muted-foreground outline-none focus:ring-1 focus:ring-primary"
              />
              <button
                onClick={handleSearch}
                className="flex h-[42px] w-[42px] items-center justify-center rounded-xl bg-primary/10 text-primary active:scale-95 transition"
              >
            ...
            {loading ? (
              ...
              <GlassCard className="flex flex-col items-center gap-2 py-10 text-center">
                ...
                <p className="text-[13px] text-muted-foreground">{t("memory.noMemory")}</p>
              </GlassCard>
            ) : (
              memories.map((m) => (
                  ...
                  {editingId === m.id ? (
                    <div className="space-y-2">
                      <textarea
                        ...
                        rows={3}
                        className="w-full rounded-lg bg-muted p-2 text-[13px] text-foreground outline-none focus:ring-1 focus:ring-primary"
                      />
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleEdit(m.id)}
                          disabled={busyId === m.id}
                          className="rounded-lg bg-primary/10 px-3 py-1.5 text-[12px] font-semibold text-primary active:scale-95 transition"
                        >
                        ...
                        <button
                          ...
                          className="rounded-lg bg-muted/60 px-3 py-1.5 text-[12px] text-muted-foreground active:scale-95 transition"
                        >
                          {t("common.cancel")}
                        </button>
                      </div>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:text-primary active:scale-95 transition"
                          >
          ...
          <div className="space-y-3">
            {proposals.length === 0 ? (
              <GlassCard className="flex flex-col items-center gap-2 py-10 text-center">
                ...
                <p className="text-[13px] text-muted-foreground">{t("memory.noProposals")}</p>
              </GlassCard>
            ) : (
              proposals.map((p) => {
                    ...
                    <div className="mt-2 space-y-1.5 text-[12px] leading-relaxed">
                      {p.finding && (
                        <p className="text-muted-foreground">
                          <span className="font-medium text-muted-foreground">发现: </span>
                          {p.finding}
                        </p>
                      )}
                      {p.proposedChange && (
                      ...
                      {p.expectedBenefit && (
                        <p className="text-muted-foreground">
                          <span className="font-medium text-muted-foreground">预期收益: </span>
                          {p.expectedBenefit}
                        </p>
                      )}
                    </div>
                    ...
                    {isPending && (
                      <div className="mt-3 flex gap-2">
                        <button
                          ...
                          className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl py-2 text-[12px] font-semibold bg-primary/10 text-primary active:scale-95 transition disabled:opacity-60"
                        >
                        ...
                        <button
                          ...
                          disabled={busyId === p.id}
                          className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl py-2 text-[12px] font-semibold bg-primary/10 text-muted-foreground active:scale-95 transition disabled:opacity-60"
                        >
