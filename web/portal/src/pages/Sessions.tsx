      ...
      <div className="page-in mx-auto max-w-3xl px-4 pb-24 pt-6">
        ...
        {loading ? (
          ...
          <GlassCard className="flex flex-col items-center gap-2 py-10 text-center">
            ...
            <p className="text-[13px] text-[var(--color-text-secondary)]">{t("sessions.noSessions")}</p>
          </GlassCard>
        ) : (
          <>
            <div className="space-y-3">
              {sessions.map((s) => (
                ...
                >
                    ...
                    >
                      <Bookmark
                        className={cn(
                          "h-4 w-4 transition",
                          s.bookmarked
                            ? "fill-[oklch(0.78_0.16_85)] text-[oklch(0.78_0.16_85)]"
                  ...
                  {/* Expanded details */}
                  {expandedId === s.id && (
                    <div className="mt-3 space-y-1.5 border-t border-white/5 pt-3 text-[12px] text-[var(--color-text-secondary)]">
                      {s.userComment && (
                        <p><span className="font-medium text-[var(--color-text-muted)]">评论: </span>{s.userComment}</p>
                      )}
                      <p><span className="font-medium text-[var(--color-text-muted)]">Session ID: </span>{s.sessionId}</p>
                      {s.completedAt && (
                        <p><span className="font-medium text-[var(--color-text-muted)]">完成时间: </span>{formatRelativeTime(s.completedAt)}</p>
                      )}
                    </div>
                  )}
                </GlassCard>
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={page === 0}
                  className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/5 text-[var(--color-text-secondary)] disabled:opacity-40 active:scale-95 transition"
                >
