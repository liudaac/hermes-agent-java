...
[x] Prompt Asset 版本演进骨架落地，支持 draft/activate 和 prompt://assetId#vN 校验
[x] smoke 脚本真实验证 Prompt Asset v2 激活与 Team Blueprint prompt://assetId#v2 引用
[x] Evolution Proposal Skeleton 落地，支持状态流转并可 apply 生成 Team Blueprint draft
...
## 17. 2026-06-16 当前阶段复盘：已进入“业务智能体团队平台”主线

本节用于记录当前真实实现状态、紧张点和后续待办。它不是新的愿景，而是对截至当前提交的工程现实做阶段性验收。

### 17.1 当前真实状态

当前 Business Portal 已经不只是页面壳，而是形成了一个可运行的业务平台 MVP：
...
```text
[x] Workspace façade：业务空间包装底层 Tenant
[x] Scenario 对象：业务场景正式落地，并绑定 Team / Run / Insights
[x] Prompt Asset：workspace 级提示词资产落地
[x] Prompt Asset Version：支持 draft / activate / activeVersion / explicit version ref
[x] Team Blueprint：版本化团队蓝图，支持 promptAssetRefs 校验
[x] Business Run：业务故事化运行记录
[x] Approval Center：审批卡、approve/reject/request-info、高风险确认短语
[x] Business Insights：基于 workspace/scenario 的最小洞察和 next actions
[x] Evolution Proposal：状态机落地，approved proposal 可 apply 生成 Team Blueprint draft
[x] /business 页面：业务工作台 MVP
[x] smoke：覆盖 workspace / prompt asset / team / run / approval / insights
```
...
mvn -q -Dtest=EvolutionProposalServiceTest,PromptAssetServiceTest,TeamBlueprintServiceTest test
...
```text
口头记录的提交号 != git log 真实提交号
旧 Dashboard 进程仍占用 9119
旧 jar / 错误启动方式导致 API 返回 401 / 404
```

后续操作必须遵守：

```text
以 git log 真实 HEAD 为准
运行 smoke 前先清理 9119 旧进程
优先用 Maven exec 启动最新源码 Dashboard
不要用无 main manifest 的普通 jar 直接启动
```
...
mvn -q -DskipTests compile exec:java \
  -Dexec.mainClass=com.nousresearch.hermes.HermesAgent \
  -Dexec.args="dashboard --port 9119 --host 127.0.0.1"
```

#### 17.2.2 /business 页面已足够，不应继续堆 UI 小修
...
创建 Approval Card
处理 Approval
展示 Home / Teams / Runs / Approvals / Insights
展示 Prompt Assets / Scenarios
```
...
#### 17.2.3 Evolution Proposal 还没有进入 smoke 和 UI

当前 Evolution Proposal 后端已落地，但还缺：

```text
smoke 覆盖
/business 页面展示 proposals
Insight → Create Proposal 的入口
Proposal → Approval Card 的真实联动
```

这是下一阶段最重要的闭环缺口。

#### 17.2.4 AgentTrace 尚未接入 BusinessRun

当前 BusinessRun 仍主要来自手动 API / UI / smoke：

```text
AgentTrace / IntentRun → BusinessRunRecord
```

这个 adapter 还未实现。没有它，Business Portal 还不是“真实运行态”的完整入口。

### 17.3 下一阶段优先级

#### P0：Evolution Proposal smoke 覆盖

目标：把刚落地的 Proposal 状态机和 apply 生成 Team Draft 的能力纳入集成验证。

建议 smoke 流程：

```text
Create Workspace
Create Prompt Asset v2
...
Approve Proposal
Apply Proposal
Verify Team Blueprint draft version created
```

验收：

```text
[x] /evolution-proposals create 返回 proposalId
[x] /evaluate 成功
[x] /request-approval 成功
[x] /approve 成功
[x] /apply 成功
[x] Team Blueprint versions 增加 draft version
```

#### P1：BusinessInsight → EvolutionProposal 入口

目标：让洞察不只显示建议，而能一键生成 proposal。

最小实现：

```text
POST /api/v1/workspaces/{workspaceId}/insights/{insightId}/proposal
或
POST /api/v1/workspaces/{workspaceId}/evolution-proposals/from-insight
```

由于当前 insights 是动态生成对象，第一版可以从请求体传入：
...
#### P1：/business 页面展示 Evolution Proposals

目标：让业务用户看到 proposal 状态机。
...
Proposal 列表
状态 badge
Evaluate / Request Approval / Approve / Reject / Apply 操作
Applied 后展示 targetDraftVersion
```

#### P2：AgentTrace → BusinessRun adapter

目标：让 Business Portal 的 Run 不再主要依赖手工创建。
...
#### P2：Prompt Asset UI 版本管理

当前 Prompt Asset 版本后端已完成，但 UI 只支持创建资产，不支持创建/激活版本。
...
```text
1. 后端刚完成 Proposal skeleton，需要集成验证固化
2. 这是原方案“洞察 → 可审批进化”的核心闭环
3. 比继续做 UI polish 更贴近平台主线
4. 能及时发现 apply → Team Draft 的真实 API 问题
```
...
### 21.16 Iteration 11：Business Portal foundation architecture test 已落地

第十一刀完成轻量 architecture test：

```text
src/test/java/com/nousresearch/hermes/business/foundation/BusinessPortalFoundationArchitectureTest.java
```

它把 adapter/facade 合同变成可执行测试。
...
暂时不约束 legacy dashboard/org handlers，避免把历史 dashboard 架构重构混进本轮 foundation 收敛。

检查规则：

```text
普通 Business Portal 类不能直接 import 低层 foundation 包
只有 business.foundation 和显式 adapter 类可以 bridge 到 foundation
business.foundation 包保持很薄，只允许 BusinessPortalFoundationFacade / BusinessPortalAdapterRegistry
```
