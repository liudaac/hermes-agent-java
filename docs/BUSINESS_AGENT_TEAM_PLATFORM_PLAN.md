...
开发按五阶段推进：
- **Phase 0.5**：MVP 合同与边界收敛，明确暂不做清单
- **Phase 1**：Workspace + Team Blueprint Versioning 最小骨架（文件持久化，API + 测试）
- **Phase 2**：Scenario 与 Run API，绑定 Team Blueprint 并记录运行轨迹
- **Phase 3**：Skill / Tool Policy 产品化，workspace/agent 级权限治理
- **Phase 4**：Evolution Proposal 最小闭环（生成 → 评估 → 审批 → 发布）
- **Phase 5**：部署与生产治理（Docker、数据库、监控、备份）
