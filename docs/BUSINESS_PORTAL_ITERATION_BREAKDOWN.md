...
### Step 4.28：Prompt Asset 后端最小版

目标：让 Team Blueprint 的 promptAssetRefs 不再只是字符串占位，先把 workspace 级 Prompt Asset 作为正式对象落地。

交付：

```text
[x] 新增 PromptAssetRecord
[x] 新增 FilePromptAssetRepository
[x] 新增 PromptAssetService
[x] 新增 PromptAssetDashboardIntegration
[x] DashboardServer 注册 PromptAssetDashboardIntegration
[x] GET /api/v1/workspaces/{workspaceId}/prompt-assets
[x] POST /api/v1/workspaces/{workspaceId}/prompt-assets
[x] GET /api/v1/workspaces/{workspaceId}/prompt-assets/{assetId}
[x] 文件持久化路径：$HERMES_HOME/business/workspaces/{workspaceId}/prompt-assets/{assetId}.json
[x] 新增 PromptAssetServiceTest
[x] 测试：创建 / 列表 / 详情 / duplicate / missing workspace
[x] mvn -q -Dtest=PromptAssetServiceTest,ScenarioServiceTest,WorkspaceServiceTest test 通过
```

当前范围：

```text
Prompt Asset 已作为 workspace 级资产落地。
